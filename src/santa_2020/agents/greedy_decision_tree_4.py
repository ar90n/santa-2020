...
AGENT_KEY = "greedy_decision_tree_4"
...
def agent(stats: BanditStats, env: Environment) -> int:
    ...
    if stats.step == 0:
        features = np.zeros((len(stats.bandits), 11))
        ...
        features[:, 4] = features[:, 1] + features[:, 3]
        features[:, 5] = features[:, 1] - features[:, 2]
        features[:, 6] = features[:, 2] / (features[:, 1] + 1e-12)
        features[:, 7] = features[:, 1] / (features[:, 3] + 1e-12)
        features[:, 8] = features[:, 1] / (features[:, 0] + 1e-12)
        features[:, 9] = features[:, 3] / (features[:, 0] + 1e-12)
        features[:, 10] = np.power(0.97, features[:, 4])
